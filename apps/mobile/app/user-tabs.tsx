...
type TabKey = 'home' | 'rights' | 'call' | 'tokens' | 'profile';
...
const TABS: TabItem[] = [
  ...
  { key: 'rights', label: 'Rights', icon: 'shield-outline', iconFocused: 'shield-check' },
  { key: 'call', label: 'Call', icon: 'phone', iconFocused: 'phone' },
  { key: 'tokens', label: 'Tokens', icon: 'wallet-outline', iconFocused: 'wallet' },
  { key: 'profile', label: 'Profile', icon: 'account-outline', iconFocused: 'account' },
];

export default function UserTabsScreen(): React.JSX.Element {
  ...
  const handleTabPress = useCallback((key: TabKey): void => {
    switch (key) {
      case 'call':
        router.push(balance <= 0 ? '/token-store' : '/scenario-select');
        break;
    ...
    <View style={styles.container}>
      ...
      <SafeAreaView edges={['bottom']} style={styles.tabBarSafe}>
        <View style={styles.tabBar}>
          {TABS.map((tab) => {
            const isCall = tab.key === 'call';
            const isActive = tab.key === 'home'; // Home is always the active tab on this screen
              ...
              return (
                ...
                >
                  <View style={styles.callButton}>
                    <Icon source="phone" color={palette.white} size={28} />
                  </View>
            ...
            return (
              ...
              >
                <Icon
                  source={isActive ? tab.iconFocused : tab.icon}
